...
class PlayerForegroundService : Service() {
    ...
    override fun onCreate() {
        ...
        voiceOverlayManager = VoiceOverlayManager(this) { broadcastState() }.apply {
            ...
            if (s.useAccessibilityMode) {
                audioCaptureMode = ame.project.kanae.overlay.AudioCaptureMode.ACCESSIBILITY
            }
    ...
    fun enableCanvasMode(px: Int, py: Int, qx: Int, qy: Int) {
        ...
        if (voiceOverlayManager.isShowing)
            voiceOverlayManager.setCanvasMode(locked = true, x = qx, y = qy + 250)
    ...
    fun disableCanvasMode() {
        ...
        if (voiceOverlayManager.isShowing) voiceOverlayManager.setCanvasMode(locked = false)
    ...
    fun updateUseAccessibilityMode(enabled: Boolean) {
        settingsManager.settings.useAccessibilityMode = enabled
        settingsManager.saveSettings()
        // Re-init voiceOverlayManager or update its mode
        voiceOverlayManager.audioCaptureMode = if (enabled) {
            ame.project.kanae.overlay.AudioCaptureMode.ACCESSIBILITY
        } else {
            ame.project.kanae.overlay.AudioCaptureMode.STANDARD
        }
        broadcastState()
    }
    ...
    fun getStateMap(): Map<String, Any?> {
        ...
        return mapOf(
            ...
            "voice_accessibility_enabled" to s.useAccessibilityMode,
