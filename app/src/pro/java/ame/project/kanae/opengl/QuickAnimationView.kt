...
package ame.project.kanae.opengl
...
/**
 * Semua efek yang tersedia. Tinggal panggil startAnimation(effect) sebanyak
 * apapun / secepat apapun (spam-friendly) — partikel dikelola lewat pool
 * berukuran tetap jadi biayanya konstan, tidak makin berat walau di-spam.
 */
...
class QuickAnimationView @JvmOverloads constructor(
    ...
    // ---------------------------------------------------------------------------------------
    // Shapes dikirim per-vertex lewat attribute float, jadi 1 draw call bisa render
    // banyak bentuk berbeda sekaligus (lingkaran, hati, bintang, diamond).
    // ---------------------------------------------------------------------------------------
    ...
    private class Particle {
        ...
        // Nilai acak serbaguna dipakai berbeda-beda tergantung efek (fase, arah, dsb).
        var seed = 0f
        var seed2 = 0f

        // Hasil hitungan tiap frame, ditulis ke buffer render.
        var curX = 0f
        var curY = 0f
        var curSize = 0f
        var curAlpha = 0f
    ...
    private class MyRenderer(private val view: GLSurfaceView) : Renderer {
        ...
        private fun fadeInOut(elapsed: Float, life: Float, fadeInDur: Float, fadeOutFrac: Float): Float {
            ...
            return kotlin.math.min(fadeIn, fadeOut)
        }
        ...
        private fun initParticle(p: Particle, effect: AnimationEffect) {
            ...
            when (effect) {
                ...
                AnimationEffect.HEARTS -> {
                    ...
                    p.r = 1f
                    p.g = Random.nextFloat() * 0.35f
                    p.b = Random.nextFloat() * 0.25f + 0.35f
                ...
                AnimationEffect.FIREWORKS -> {
                    ...
                    p.vx = cos(angle) * speed
                    p.vy = sin(angle) * speed
                    p.r = Random.nextFloat() * 0.4f + 0.6f
                    p.g = Random.nextFloat() * 0.4f + 0.6f
                    p.b = Random.nextFloat() * 0.4f + 0.6f
                ...
                AnimationEffect.SPARKLE -> {
                    p.x0 = Random.nextFloat() * 1.6f - 0.8f
                    p.y0 = Random.nextFloat() * 1.6f - 0.8f
                    p.vx = 0f
                    p.vy = Random.nextFloat() * 0.06f + 0.02f
                    p.r = Random.nextFloat() * 0.3f + 0.7f
                    p.g = Random.nextFloat() * 0.3f + 0.7f
                    p.b = Random.nextFloat() * 0.3f + 0.7f
                ...
                AnimationEffect.BUBBLES -> {
                    p.x0 = Random.nextFloat() * 1.8f - 0.9f
                    p.y0 = -1.1f
                    p.vx = Random.nextFloat() * 0.1f - 0.05f
                    p.vy = Random.nextFloat() * 0.2f + 0.15f
                    p.r = Random.nextFloat() * 0.3f + 0.4f
                    p.g = Random.nextFloat() * 0.3f + 0.7f
                    p.b = 1f
                ...
                AnimationEffect.GLITTER -> {
                    ...
                    if (Random.nextBoolean()) {
                        p.r = 1f; p.g = 0.84f; p.b = 0.2f // emas
                    } else {
                        p.r = 0.85f; p.g = 0.9f; p.b = 0.95f // perak
                    }
                ...
                AnimationEffect.PETALS -> {
                    ...
                    p.y0 = 1.1f
                    p.vx = Random.nextFloat() * 0.3f - 0.15f
                    p.vy = -(Random.nextFloat() * 0.2f + 0.2f)
                    p.r = Random.nextFloat() * 0.3f + 0.7f
                    p.g = Random.nextFloat() * 0.3f + 0.3f
                    p.b = Random.nextFloat() * 0.3f + 0.6f
                    p.baseSize = Random.nextFloat() * 10f + 10f
                ...
                AnimationEffect.LEAVES -> {
                    ...
                    p.vy = -(Random.nextFloat() * 0.3f + 0.2f)
                    p.r = Random.nextFloat() * 0.3f + 0.1f // Greenish/Brownish
                    p.g = Random.nextFloat() * 0.4f + 0.4f
                ...
                AnimationEffect.STARDUST -> {
                    p.x0 = Random.nextFloat() * 2f - 1f
                    p.y0 = Random.nextFloat() * 2f - 1f
                    p.vx = Random.nextFloat() * 0.1f - 0.05f
                    p.vy = Random.nextFloat() * 0.1f - 0.05f
                    p.r = 1f; p.g = 1f; p.b = 0.5f // Golden/Yellow
                ...
                AnimationEffect.METEOR -> {
                    p.x0 = Random.nextFloat() * 1.5f + 0.5f // Top rightish
                    p.y0 = 1.1f
                    p.vx = -1.2f - Random.nextFloat() * 0.8f
                    p.vy = -0.8f - Random.nextFloat() * 0.6f
                    p.r = 0.8f; p.g = 0.9f; p.b = 1f // Cyan/White
