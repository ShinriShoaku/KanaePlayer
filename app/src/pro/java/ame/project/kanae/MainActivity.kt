...
import ame.project.kanae.utils.AccessibilityOnboardingHelper
...
class MainActivity : AppCompatActivity() {
    ...
    private fun setupBottomSheetButtons(b: BottomSheetOverlaysBinding, dialog: BottomSheetDialog) {
        ...
        b.extraVoiceSettings.cbUseAccessibility.setOnCheckedChangeListener { buttonView, isChecked ->
            if (isChecked && !AccessibilityOnboardingHelper.isServiceEnabled(this)) {
                buttonView.isChecked = false
                MaterialAlertDialogBuilder(this)
                    .setTitle("Aktifkan Accessibility Service")
                    .setMessage("Untuk menggunakan Mode Prioritas, Anda harus mengaktifkan 'Kanae Accessibility Service' di pengaturan sistem.")
                    .setPositiveButton("Buka Pengaturan") { _, _ ->
                        AccessibilityOnboardingHelper.openAccessibilitySettings(this)
                    }
                    .setNegativeButton("Batal", null)
                    .show()
            } else {
                service?.updateUseAccessibilityMode(isChecked)
            }
        }
        ...
        fun openSettings(key: String, title: String) {
            ...
            if (key == "chat") {
                ...
                b.extraVoiceSettings.cbUseAccessibility.isChecked = s.useAccessibilityMode
        ...
        b.panelGrid.btnCustomOverlay.setOnClickListener { startActivity(Intent(this@MainActivity, CustomWebOverlaySettingsActivity::class.java)); dialog.dismiss() }
        b.panelGrid.btnMapping.setOnClickListener { startActivity(Intent(this@MainActivity, MappingActivity::class.java)); dialog.dismiss() }
        b.panelGrid.btnGesture.setOnClickListener { startActivity(Intent(this@MainActivity, QuickOverlayActivity::class.java)); dialog.dismiss() }
    ...
    private fun syncBottomSheetOnly(svc: PlayerForegroundService, state: Map<String, Any?>) {
        overlaySheetBinding?.let { b ->
            ...
            if (currentConfigKey == "voice" && b.panelSettings.visibility == View.VISIBLE) {
                val accEnabled = state["voice_accessibility_enabled"] as? Boolean ?: false
                if (b.extraVoiceSettings.cbUseAccessibility.isChecked != accEnabled) b.extraVoiceSettings.cbUseAccessibility.isChecked = accEnabled
            }
        }
    }
